...
function unwrapError(err) {
  ...
  if (!message && data && typeof data === 'object') {
    const fieldErrors = Object.values(data).filter((value) => value)
...
export async function otpSignup({ full_name, email, password, agreed_to_terms }) {
  const HARD_TIMEOUT_MS = parseInt(
    import.meta.env.VITE_OTP_SIGNUP_TIMEOUT_MS || import.meta.env.VITE_API_TIMEOUT_MS || '120000',
    10,
  )
  ...
  try {
    const { data } = await axios.post(
      '/auth/otp/signup/',
      { full_name, email, password, agreed_to_terms },
