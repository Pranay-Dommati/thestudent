...
const ScribPaidAnalytics = ({ isDarkMode = false }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await authService.makeAuthenticatedRequest('/scrib/admin/paid-analytics/');
      setData(res.data);
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`text-xs mt-0.5 ${textSub}`}>Live data — all users with at least one successful payment</p>
        </div>
        <button
          onClick={fetchData}
