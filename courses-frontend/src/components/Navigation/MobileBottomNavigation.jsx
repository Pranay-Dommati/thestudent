...
import {
    FaHome, FaGraduationCap, FaBook, FaAward, FaArrowLeft
} from 'react-icons/fa';
...
const MobileBottomNavigation = () => {
    ...
    const getNavigationConfig = () => {
        ...
        // Course Detail Page (e.g., /6th, /engineering)
        if (path.match(/^\/(6th|7th|8th|9th|10th|11th|12th|engineering)$/)) {
            return {
                type: 'course-category',
                items: [
                    { action: goBack, icon: FaArrowLeft, label: 'Back', isAction: true },
                    { path: '/', icon: FaGraduationCap, label: 'Courses', activeColor: 'text-blue-600', bgColor: 'bg-blue-50', activeBg: 'bg-blue-600' },
                    { path: '/learning-path', icon: FaBook, label: 'Create', activeColor: 'text-blue-600', bgColor: 'bg-blue-50', activeBg: 'bg-blue-600' },
                    ...(isLoggedIn ? [{ path: '/learning-hub', icon: FaAward, label: 'Hub', activeColor: 'text-blue-600', bgColor: 'bg-blue-50', activeBg: 'bg-blue-600' }] : []),
                ]
            };
        }

        // Subject Detail Page (e.g., /6th/cbse/math)
        if (path.match(/^\/(6th|7th|8th|9th|10th|11th|12th|engineering)\/[^\/]+\/[^\/]+$/)) {
            return {
                type: 'subject-detail',
                items: [
        ...
        // Learning Page (e.g., /6th/cbse/math/learning)
        // Use precise match to avoid catching "/learning-hub"
        const isCourseLearning = /^\/(6th|7th|8th|9th|10th|11th|12th|engineering)\/.+\/learning(\/|$)/.test(path);
        if (isCourseLearning) {
            return {
                type: 'learning',
                items: [
    ...
    return (
        <>
            <div
                className={`fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 lg:hidden transition-transform duration-300 ease-in-out ${isVisible ? 'translate-y-0' : 'translate-y-full'
                    }`}
                style={{
                    ...
                    {navConfig.items.map((item, index) => {
                        ...
                        return (
                            <Link
                                key={index}
                                to={item.path}
                                className={`flex flex-col items-center py-0.5 px-2 relative transition-all duration-300 rounded-lg active:scale-95 ${active ? 'transform scale-105' : 'hover:bg-gray-50'
                                    }`}
                            >
                                {/* Icon container with enhanced animations */}
                                <div className={`p-1.5 rounded-lg transition-all duration-300 hover:scale-110 ${active
                                ...
                                <span className={`text-xs font-medium mt-0.5 transition-all duration-300 ${active
