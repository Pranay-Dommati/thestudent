...
const SeventhStandard = () => {
    ...
    const handleBoardSelect = async (boardId) => {
        if (boardId === 'state') {
            ...
            navigate(`/7th/${boardId}`);
        }
    };

    const handleStateSelect = (stateId) => {
        navigate(`/7th/state/${stateId}`);
        setShowStateBoards(false);
        setSelectedBoard(`state-${stateId}`);
    };
                                    ...
                                    return (
                                        <Link
                                            to={`${(selectedBoard.includes('state')
                                                ? `/7th/state/${stateId || selectedBoard.replace('state-', '')}/${course.subject.toLowerCase()}`
                                                : `/7th/${selectedBoard}/${course.subject.toLowerCase()}`)}?courseId=${encodeURIComponent(course.id)}`}
                                            key={course.id}
                                        >
                                            <motion.div
