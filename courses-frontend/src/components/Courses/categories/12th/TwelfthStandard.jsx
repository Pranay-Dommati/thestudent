...
const TwelfthStandard = () => {
    ...
    const handleBoardSelect = async (boardId) => {
        if (boardId === 'state') {
            ...
            navigate(`/12th/${boardId}`);
        }
    };

    const handleStateSelect = (stateId) => {
        const to = `/12th/state/${stateId}`;
                                    ...
                                    return (
                                        <Link
                                            to={`${(selectedBoard.includes('state')
                                                ? `/12th/state/${stateId || selectedBoard.replace('state-', '')}/${course.subject.toLowerCase()}`
                                                : `/12th/${selectedBoard}/${course.subject.toLowerCase()}`)}?courseId=${encodeURIComponent(course.id)}`}
                                            key={course.id}
                                        >
                                            <motion.div
