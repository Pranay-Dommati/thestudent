...
const CourseHero = () => {
    ...
    return (
        <section className="bg-gradient-to-r from-indigo-600 to-purple-700 text-white pt-16 md:pt-20 pb-14 relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
                    {highlights.map((item, index) => (
                        <div
                            key={index}
                            className="text-center p-3 bg-white/10 backdrop-blur-sm rounded-lg border border-white/15 hover:bg-white/15 transition-all duration-300"
                        >
                            <item.icon className="w-5 h-5 mx-auto mb-2 text-white/90" />
                            <h3 className="text-sm font-medium mb-1">{item.title}</h3>
                            <p className="text-[10px] opacity-70 hidden md:block leading-relaxed">{item.description}</p>
                        </div>
                    ))}
                </div>
            </div>
