...
/**
 * Validates if sanitized reading is ready for the current topic
 * @param {string} sanitizedReading - Sanitized content
 * @param {string} sanitizedReadingTopicName - Topic name for sanitized content
 * @param {string} currentTopicName - Current active topic name
 * @param {object} content - Content object
 * @param {string} contentTopicName - Topic name for content
 * @returns {boolean}
 */
export const isReadingReadyForTopic = (
  sanitizedReading,
  ...
  // Check if we have sanitized content for the current topic
  const hasSanitizedForTopic = (
    sanitizedReadingTopicName === currentTopicName &&
    typeof sanitizedReading === 'string' &&
    sanitizedReading.trim().length > 0
  );

  // Check if we have raw reading for current topic (fallback)
  const hasRawReadingForTopic = (
  ...
  return hasSanitizedForTopic || hasRawReadingForTopic;
};
