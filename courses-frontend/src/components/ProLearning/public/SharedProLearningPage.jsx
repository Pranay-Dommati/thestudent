...
const TabButton = ({ active, onClick, icon: Icon, children }) => (
    <button
        onClick={onClick}
        className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${active ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
            }`}
    >
        <Icon size={16} />
        {children}
        ...
        return (
            <div className="max-w-4xl mx-auto px-4 py-10">
                <div className="bg-red-50 border border-red-200 text-red-800 rounded-xl p-4">
                    ...
                    <div className="mt-3">
                        <Link to="/learning-path" className="text-sm text-red-700 underline">Go to Learn Smarter</Link>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-6">
            <div className="flex items-center justify-between mb-4">
                <Link to="/learning-path" className="text-gray-600 hover:text-gray-800 flex items-center gap-2 text-sm">
                    <IoChevronBack /> Back to Learn Smarter
                </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="md:col-span-1 bg-white rounded-xl border border-gray-200 p-3 h-fit sticky top-4">
                    ...
                    <div className="space-y-1">
                        {(course?.topics || []).map(t => (
                            <button key={t.id} onClick={() => setSelectedTopicId(t.id)} className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${selectedTopicId === t.id ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'
                                }`}>
                                {t.topic_name || t.name}
                            </button>
                        ))}
