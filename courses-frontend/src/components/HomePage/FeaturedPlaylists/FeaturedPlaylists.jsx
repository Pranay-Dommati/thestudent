...
const FeaturedPlaylists = () => {
    ...
    const handleCourseClick = (course) => {
        ...
        if (course.courseType === 'engineering') {
            const navigationPath = `/engineering/${course.id}`;
            // Removed debug log
        ...
        if (boardPath === 'state' && course.state) {
            // For state board, include state abbreviation: /{class}/state/{state}/{subject}
            const stateShortNames = {
                'Andhra Pradesh': 'ap',
                'Telangana': 'ts',
                'Tamil Nadu': 'tn',
            ...
            navigationPath = `/${classPath}/state/${stateCode}/${subjectPath}`;
        } else {
            // For CBSE and other boards: /{class}/{board}/{subject}
            navigationPath = `/${classPath}/${boardPath}/${subjectPath}`;
        }
