                ...
                <div className="flex px-2 gap-1 lg:gap-2 min-w-full">
                    {days.map(day => (
                        <button
                            ...
                            className={`px-3 lg:px-4 py-1.5 lg:py-2 rounded-full text-xs lg:text-sm font-medium whitespace-nowrap flex-shrink-0 ${activeDay === day.number
...
const ChatbotPage = () => {
    ...
    useEffect(() => {
        ...
        const handleShowSaveModal = (event) => {
            ...
            console.log('📢 Received freemium save modal event:', courseData);
    ...
    // Freemium: Auto-save course when user logs in
    useEffect(() => {
        const authed = typeof isAuthenticated === 'function' ? isAuthenticated() : !!isAuthenticated;
        ...
        if (authed && user && !loading && freemiumLoginInProgress) {
            ...
            if (pendingCourse) {
                try {
                    ...
                    console.log('💾 User logged in! Auto-saving freemium course:', courseData);
    ...
    const handleTopicConfirm = async () => {
        ...
        try {
            ...
            if (!authed) {
                ...
                console.log('💾 Stored pending freemium course for later save:', courseData);

                // Increment the free courses counter
    ...
    return (
        <div className="h-screen flex overflow-hidden w-full bg-white relative">
            ...
            {/* Sidebar */}
            <div className={`fixed inset-y-0 left-0 z-30 transform transition-transform duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
            ...
            {/* Main chat container */}
            <div className={`flex-1 flex flex-col h-full w-full relative overflow-hidden transition-all duration-300 ${isSidebarOpen ? "lg:ml-64" : "ml-0"
                }`}>
                                ...
                                <div className="relative group overflow-visible">
                                    ...
                                    <div className="absolute right-2 top-3 bottom-4 flex items-center">
                                        <button
                                            ...
                                            className={`h-9 w-9 flex items-center justify-center rounded-xl transition-all shadow-sm ${message.trim() && !isLoading && online
