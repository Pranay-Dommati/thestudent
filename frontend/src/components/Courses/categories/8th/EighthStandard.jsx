...
const EighthStandard = () => {
  ...
  useEffect(() => {
    setAvailableBoards(getOptimisticBoardAvailability('8th'));
  ...
  const handleBoardSelect = (boardId) => {
    if (boardId === 'state') {
      ...
      navigate(`/courses/8th/${boardId}`);
    }
  };

  const handleStateSelect = (stateId) => {
    navigate(`/courses/8th/state/${stateId}`);
    setShowStateBoards(false);
    setSelectedBoard(`state-${stateId}`);
  };
                ...
                return (
                  <Link 
                    to={`${(selectedBoard.includes('state') 
                      ? `/courses/8th/state/${stateId || selectedBoard.replace('state-', '')}/${course.subject.toLowerCase()}` 
                      : `/courses/8th/${selectedBoard}/${course.subject.toLowerCase()}`)}?courseId=${encodeURIComponent(course.id)}`}
                    key={course.id}
                  >
                    <motion.div 
